...
struct Point {
    ...
    bool   operator <(const Point  &p) const {
        if (sig(x-p.x) == 0)
            return sig(y-p.y) < 0 || (sig(y-p.y) == 0 && sig(z-p.z) < 0);
        return sig(x-p.x) < 0;
    }
    ...
    bool      parallel(Point v) {return sig((*this^v).len()) == 0;}
    bool perpendicular(Point v) {return sig(*this*v) == 0;}
...
struct Line {
    ...
    // 保证点不在线上
    bool     same_side(Point p1, Point p2) {return sig(((b-a)^(p1-a)) * ((b-a)^(p2-a))) > 0;}
    bool opposite_side(Point p1, Point p2) {return sig(((b-a)^(p1-a)) * ((b-a)^(p2-a))) < 0;}
...
struct Plane {
    ...
    bool     same_side(Point p1, Point p2) {return sig(normal()*(p1-a) * (normal()*(p2-a))) > 0;}

    // 平面与直线 平行/垂直
    bool      parallel(Line  l) {return sig(normal()*(l.b-l.a)) == 0;}
    bool perpendicular(Line  l) {return sig((normal()^(l.b-l.a)).len()) == 0;}
